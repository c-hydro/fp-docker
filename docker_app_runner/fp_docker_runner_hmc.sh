...
docker run -d \ 
--name=${container_name} \
--mount source=volume_dte_in,target=${TARGET_DATA_STATIC} \
${image_docker_reference}:${image_docker_version}
	...
	#--mount type=bind,source=${SOURCE_DATA_DYNAMIC_SOURCE},target=${TARGET_DATA_DYNAMIC_SOURCE}\
 	...
 	#--mount type=volume,source=volume_dte_in,target=${TARGET_DATA_DYNAMIC_RESTART}\
	#--mount type=volume,source=volume_dte_in,target=${TARGET_DATA_DYNAMIC_SOURCE}\
 	#--mount source=volume_dte_out,target=${TARGET_DATA_DYNAMIC_DESTINATION} \
 	#--mount source=volume_dte_out,target=${TARGET_DATA_DYNAMIC_STATE} \
